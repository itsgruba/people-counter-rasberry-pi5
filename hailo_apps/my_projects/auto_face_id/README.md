...
- `visit_events`: chronological `entry`/`exit` rows, each linked to a person and snapshot
- `entered_person`: current people inside, used by the exit camera to match exits
...
- every confirmed entry/exit saves a JPEG under
  `samples/<person>/visit_events/visit_<N>/<entry|exit>/`
...
SELECT persons.label, visit_events.event_type, visit_events.visit_number,
       visit_events.event_at, visit_events.photo_path
FROM visit_events
JOIN persons ON persons.id = visit_events.person_id
ORDER BY visit_events.event_at DESC;
```

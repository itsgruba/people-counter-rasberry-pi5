...
В полевом режиме embedding и JPEG оцениваются отдельно. Каждый технически
корректный 512-мерный embedding сохраняется во временный буфер, даже если кадр
не прошёл пороги резкости, ракурса или confidence для сохранения фотографии.
После A → B одного embedding достаточно для создания распознаваемой записи;
до пересечения сохраняются лучшие `--max-pending-embeddings` кандидатов.
Placeholder продолжает принимать embeddings текущего трека и становится
доступным поиску сразу после первого удачного backfill. На EXIT требуется
`--recognition-vote-threshold` последовательных голосов за одного и того же
наиболее близкого человека из находящихся внутри. Минимального порога confidence
нет: это позволяет работать с масками и частично закрытым лицом, а несколько
голосов защищают от решения по одному случайному кадру.

Привязка `track_id → global_id` существует только пока жив текущий трек камеры.
Если трек отсутствует более `--identity-track-max-gap-frames` кадров (по умолчанию
8), его identity и состояние пересечения удаляются. Поэтому повторно пришедший
человек на ENTRY всегда создаёт новую запись, даже если Hailo переиспользовал
старый числовой `track_id`. Поиск по другим записям базы выполняется только EXIT
и только среди строк `entered_person`.
...
Для длительной работы watchdog и встроенный supervisor включены по умолчанию.
Если кадры не проходят 20 секунд либо live RTSP/Hailo pipeline выдаёт ERROR/EOS,
worker завершается и supervisor запускает новый чистый процесс с теми же
аргументами. Hailo pipeline не пересобирается внутри зависшего процесса: такой
rebuild может зависнуть при освобождении native Hailo/GStreamer threads. ENTRY
перезапускается через 2 секунды, EXIT — через 4 секунды, чтобы два процесса не
открывали один Hailo-10H одновременно; задержку меняет
`--recovery-delay-seconds`. Порог зависания меняет
`--watchdog-timeout-seconds`. Ошибка обработки отдельного кадра записывается в
лог и не останавливает поток. Отключить watchdog можно явно через
`--disable-watchdog` (для рабочего запуска это не рекомендуется).

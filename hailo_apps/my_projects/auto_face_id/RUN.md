...
Для длительной работы watchdog включён по умолчанию. Если кадры перестают
проходить, либо live RTSP/Hailo pipeline выдаёт ERROR/EOS, приложение пересобирает
pipeline. ENTRY ждёт 2 секунды, EXIT — 4 секунды, чтобы два процесса не открывали
один Hailo-10H одновременно; задержку меняет `--recovery-delay-seconds`. Ошибка
обработки отдельного кадра записывается в лог и не останавливает поток. Отключить
восстановление можно только явно через `--disable-watchdog` (для рабочего запуска
